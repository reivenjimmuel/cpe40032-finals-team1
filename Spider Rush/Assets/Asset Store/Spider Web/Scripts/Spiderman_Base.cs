...
public class Spiderman_Base : MonoBehaviour {
    ...
    public void PlayShootWebAnimation(Vector3 dir, Action onAnimComplete) {
        if (unitAnimation.GetActiveAnimType().GetName() == "Spiderman_ShootWebSingleTwo") {
            unitAnimation.PlayAnimForced(UnitAnimType.GetUnitAnimType("Spiderman_ShootWebSingleOne"), dir, 1.5f, (UnitAnim unitAnim) => {
                if (onAnimComplete != null) onAnimComplete();
            }, null, null);
        } else {
            unitAnimation.PlayAnimForced(UnitAnimType.GetUnitAnimType("Spiderman_ShootWebSingleTwo"), dir, 1.5f, (UnitAnim unitAnim) => {
                if (onAnimComplete != null) onAnimComplete();
            }, null, null);
        }
    }
